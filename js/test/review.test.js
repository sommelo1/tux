...
test('review start → proxy injects client → status → persistence across restart → stop', async () => {
  ...
  // 1) start (attach mode)
  const started = tux(['review', 'start', '--url', `http://127.0.0.1:${appPort}`, '--port', '4186', '--session', 'integration'], work);
  ...
  cleanups.push(() => tux(['review', 'stop', '--format', 'json'], work));
  ...
  const status1 = JSON.parse(tux(['review', 'status', '--format', 'json'], work).stdout);
  ...
  const stopped = JSON.parse(tux(['review', 'stop', '--format', 'json'], work).stdout);
  assert.equal(stopped.stopped, true);
  await new Promise((r) => setTimeout(r, 500));
  const status2 = JSON.parse(tux(['review', 'status', '--format', 'json'], work).stdout);
  ...
  const again = tux(['review', 'start', '--url', `http://127.0.0.1:${appPort}`, '--port', '4186'], work);
  assert.equal(again.status, 0, again.stderr);
  cleanups.pop(); // replace the guard cleanup: stop once at the end is enough
  await waitFor('http://127.0.0.1:4186/api/tux/health');
  const status3 = JSON.parse(tux(['review', 'status', '--format', 'json'], work).stdout);
  assert.equal(status3.feedback_count, 1);
  tux(['review', 'stop'], work);
});

test('review start --dry-run prints the plan without side effects (SPC 39)', async () => {
  const work = mkdtempSync(join(tmpdir(), 'tux-dry-'));
  cleanups.push(() => rmSync(work, { recursive: true, force: true }));
  const r = tux(['review', 'start', '--url', 'http://localhost:3000', '--dry-run', '--format', 'json'], work);
...
test('review start without url/cmd and no config store is rejected explicitly', async () => {
  const work = mkdtempSync(join(tmpdir(), 'tux-bad-'));
  cleanups.push(() => rmSync(work, { recursive: true, force: true }));
  // spawn-mode without a reachable target must fail with a server error (exit 4)
  const r = spawnSync(process.execPath, [bin, 'review', 'start', '--', 'node', '-e', 'setTimeout(()=>{},30000)'], {
